...
export default function () {
  ...
  const getProducts = async () => {
    ...
    setCompras(ordenes);
    setStatus(ordenes.map((elem: any) => "0"));
  };

  const handleStatus = (event: React.ChangeEvent<HTMLSelectElement>, orderId: string, orderIndex: number) => {
    const selected = event.target.value as "2" | "1" | "0";

    urlAxios.patch(`/delivery/${orderId}`, { vendorId: id, status: selected }).then(elem => console.log(elem))
      .then(() => {
          ...
          {compras.map((order: any, orderIndex: number) => (
            <div className={styles.ordenes} key={orderIndex}>
              ...
              <form className={styles.contForm}>
                ...
                <select value={status[orderIndex]} onChange={(e) => handleStatus(e, order._id, orderIndex)}>
